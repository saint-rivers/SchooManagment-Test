...
@WebServlet({ "/UpdateClassStudy", "/UpdateClassServlet" })
public class UpdateClassStudy extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public UpdateClassStudy() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String msg = ClassStudyService.updateClassStudy(csb);
		HttpSession session = request.getSession(true);
		session.setAttribute("code", msg);
		response.sendRedirect("./view/update/class?id=" + csb.getClass_id() );
	}
