...
(function (factory) {
	...
	var Time = function (hour, minute) {
		...
		this.invert = function () {
			if (this.getPeriod() === 'ព្រឹក') this.setHour(this.getHour() + 12);
		...
		this.getPeriod = function () { return this.hour < 12 ? 'ព្រឹក' : 'ថ្ងៃ'; };
	};
	...
	var MDTimePicker = function (el, config) {
		...
		this.timepicker = {
			...
			timeHolder: {
				...
				am_pm: $('<span class="mdtp__ampm">ព្រឹក</span>')
			},
			clockHolder: {
				wrapper: $('<section class="mdtp__clock_holder"></section>'),
				am: $('<span class="mdtp__am">ព្រឹក</span>'),
				pm: $('<span class="mdtp__pm">ថ្ងៃ</span>'),
				clock: {
	...
	MDTimePicker.prototype = {
				...
				hour.find('span').click(function () {
					var _hour = parseInt($(this).parent().data('hour')),
						...
						_value = _.config.is24hour ? _hour :
							(_hour + ((_selectedT === 'ថ្ងៃ' && _hour < 12) || (_selectedT === 'ព្រឹក' && _hour === 12) ? 12 : 0)) % 24,
		...
		setPeriod: function (period) {
			...
			this.timepicker.clockHolder.am[_period === 'ព្រឹក' ? 'addClass' : 'removeClass']('active');
			this.timepicker.clockHolder.pm[_period === 'ថ្ងៃ' ? 'addClass' : 'removeClass']('active');
		},
